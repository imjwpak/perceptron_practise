...
class IrisModel:
    def __init__(self):
        self._iris = pd.read_csv('https://archive.ics.uci.edu/ml/machine-learning-databases/iris/iris.data',
                                 header=None)
        # setosa와 versicolor 선택
        t = self._iris.iloc[0:100, 4].values
        self.y = np.where(t == 'Iris-setosa', -1, 1)

        # 꽃받침 길이와 꽃잎을 추출
        self.X = self._iris.iloc[0:100, [0,2]].values
    ...
    def plot_decision_regions(self):
        ...
        resolution = 0.2
        """
        <numpy 모듈의 arrange 함수>
        
        반열린 구간 [start, stop]에서
        step의 크기만큼 일정하게 떨어져 있는 숫자들을
        array 형태로 반환해 주는 함수
        
        <meshgreid 명령>
        사각형 영역을 구성하는 가로축의 점들과 세로축의 점을 나타내는
